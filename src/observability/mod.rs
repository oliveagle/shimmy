...
impl ObservabilityManager {
    ...
    pub async fn record_request(&self, model_name: &str, response_time: Duration, success: bool) {
        ...
        let model_metrics = metrics.model_stats
        ...
        debug!("Recorded request for model '{}': {}ms, success: {}", 
    ...
    pub async fn optimize_system(&self) -> Result<Vec<OptimizationAction>> {
        ...
        if optimization_state.last_optimization.elapsed().unwrap_or_default() < Duration::from_secs(300) {
            return Ok(actions);
        }
        ...
        let popular_models: Vec<_> = metrics.model_stats
            .iter()
    ...
    async fn export_prometheus(&self, metrics: &SystemMetrics) -> String {
        ...
        // Request metrics
        output.push_str(&format!("shimmy_requests_total {}\n", metrics.total_requests));
        output.push_str(&format!("shimmy_requests_successful {}\n", metrics.successful_requests));
        output.push_str(&format!("shimmy_requests_failed {}\n", metrics.failed_requests));
        output.push_str(&format!("shimmy_response_time_avg {}\n", metrics.average_response_time));
        
        // Resource metrics
        output.push_str(&format!("shimmy_memory_usage_mb {}\n", metrics.memory_usage_mb));
        output.push_str(&format!("shimmy_cpu_usage_percent {}\n", metrics.cpu_usage_percent));
        ...
        for (model, stats) in &metrics.model_stats {
            output.push_str(&format!("shimmy_model_requests{{model=\"{}\"}} {}\n", model, stats.requests));
            output.push_str(&format!("shimmy_model_errors{{model=\"{}\"}} {}\n", model, stats.errors));
            output.push_str(&format!("shimmy_model_popularity{{model=\"{}\"}} {}\n", model, stats.popularity_score));
        }
    ...
    async fn export_human_readable(&self, metrics: &SystemMetrics) -> String {
        ...
        output.push_str(&format!("🚀 Requests: {} total ({:.1}% success)\n", 
                                metrics.total_requests, success_rate));
        output.push_str(&format!("⚡ Avg Response Time: {:.1}ms\n", metrics.average_response_time));
        ...
        sorted_models.sort_by(|a, b| b.1.popularity_score.partial_cmp(&a.1.popularity_score).unwrap());
        
        for (model, stats) in sorted_models.iter().take(5) {
            output.push_str(&format!("  • {}: {} requests ({:.1} score)\n", 
...
mod tests {
    ...
    async fn test_request_recording() {
        ...
        obs.record_request("phi3-mini", Duration::from_millis(150), true).await;
        obs.record_request("phi3-mini", Duration::from_millis(200), true).await;
    ...
    async fn test_metrics_export() {
        ...
        obs.record_request("test-model", Duration::from_millis(100), true).await;
    ...
    async fn test_optimization_suggestions() {
        ...
        for _ in 0..10 {
            obs.record_request("slow-model", Duration::from_millis(1500), true).await;
        }
